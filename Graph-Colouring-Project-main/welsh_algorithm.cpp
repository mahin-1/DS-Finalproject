...
void print(string str, int *colour)
{
    ...
    string filename = "proc_welsh_algo" + to_string(pid) + ".log";
...
void getGraph(Graph *graph)
{
    ...
    for(int i = 1; i < n; i++)
    {
        if(find(graph->adj[highest].begin(), graph->adj[highest].end(), i) != graph->adj[highest].end())
        {
            initialising_set.insert(i);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        if (initialising_set.find(pid) != initialising_set.end())
        {
            colour[pid] = 1;
